...
public class FacebookHelper
{
    public static async Task<string> LoginMFacebook(string uid, string pass, string code2Fa, string ua)
    {
        try
        {
            ...
            using (HttpRequest request = new HttpRequest())
            {
                ...
                string url = response.Address.AbsoluteUri;
    ...
    public static async Task<string> CheckPoint(AccountErrorModel account)
    {
        try
        {
            using (HttpRequest request = new HttpRequest())
            {
                ...
                if (url.Contains("checkpoint"))
                {
                    ...
                    request.AddHeader("accept-language", "en-US,en;q=0.9");
                    request.AddHeader("dpr", "1");
                    request.AddHeader("referer", "https://d.facebook.com/");
                    request.AddHeader("sec-ch-prefers-color-scheme", "dark");
                    request.AddHeader("sec-ch-ua-mobile", "?0");
                    request.AddHeader("sec-ch-ua-model", "\"\"");
                    request.AddHeader("sec-fetch-dest", "document");
                    ...
                    if (matches.Any())
                    {
                        ...
                        {
                            if (match.Value.Contains("stepper"))
                            {
                                url = "https://d.facebook.com/" + match.Value;
                                url = HttpUtility.HtmlDecode(url);
                                request.AddHeader("authority", "d.facebook.com");
                                request.AddHeader("accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7");
                                request.AddHeader("accept-language", "en-US,en;q=0.9");
                                ...
                                if (matches.Any())
                                {
                                    ...
                                    {
                                        if (o.Value.Contains("anti_scripting"))
                                        {
                                            url = "https://d.facebook.com/" + o.Value;
                                            url = HttpUtility.HtmlDecode(url);
                                            request.AddHeader("authority", "d.facebook.com");
                                            request.AddHeader("accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7");
                                            request.AddHeader("accept-language", "en-US,en;q=0.9");
                                            ...
                                            if (m.Success)
                                            {
                                                ...
                                                string fbdtsg = Regex.Match(html, @"name=""fb_dtsg"" value=""(.*?)""")
                                                    .Groups[1].Value;
                                                string jazoest = Regex.Match(html, @"name=""jazoest"" value=""(.*?)""")
                                                    .Groups[1].Value;
                                                string challenge = "email_captcha";

                                                var urlParams = new RequestParams();

                                                urlParams["jazoest"] = jazoest;
                                                urlParams["fb_dtsg"] = fbdtsg;
                                                urlParams["challenge"] = challenge;
                                                ...
                                                if (m.Success)
                                                {
                                                    ...
                                                    request.AddHeader("viewport-width", Random.Shared.Next(500, 2000) + "");
                                                     ...
                                                     fbdtsg = Regex.Match(html, @"name=""fb_dtsg"" value=""(.*?)""")
                                                        .Groups[1].Value;
                                                     jazoest = Regex.Match(html, @"name=""jazoest"" value=""(.*?)""")
                                                        .Groups[1].Value;
                                                    ...
                                                    if (m.Success)
                                                    {
                                                        ...
                                                        var code =await EmailHelper.ReadEmailAsync(account.Email,
                                                            account.PassEmail);
    ...
    private static void SaveOpenFile(string? html)
    {
#if DEBUG
        try
        {
    ...
    public static bool CheckLogin(string uid, string pass, string code2fa, string ua)
    {
        using (HttpRequest request = new HttpRequest())
        {
            ...
            request.AddHeader("upgrade-insecure-requests", "1");
            request.AddHeader("user-agent", ua);
            request.AddHeader("viewport-width", Random.Shared.Next(800, 2000) + "");

            var response = request.Get("https://m.facebook.com/login.php");
            ...
            html = response.ToString();
            var url = response.Address.AbsoluteUri;
